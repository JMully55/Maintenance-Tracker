...
function calculateDueDate(lastComp, freqDays, isOneTime) {
    ...
    const date = createLocalDate(lastComp);
    date.setDate(date.getDate() + parseInt(freqDays));
    return date;
}
...
function getRecurringDueDates(task, mStart, mEnd) {
    ...
    let curr = calculateDueDate(task.lastCompleted, task.frequencyDays, task.isOneTime);
    while (curr && curr.getTime() <= mEnd.getTime()) {
        if (curr.getTime() >= mStart.getTime()) {
            events[formatDate(curr)] = { name: task.taskName + (task.isOneTime ? ' (1-Time)':''), overdue: curr.getTime() < getToday() };
        }
        if (task.isOneTime) break;
        curr.setDate(curr.getDate() + parseInt(task.frequencyDays));
    }
    return events;
}
...
window.renderCalendar = function() {
    ...
    for (let i=0; i<42; i++) {
        const dDiv = document.createElement('div'); dDiv.className='calendar-day' + (curr.getMonth()!==m?' empty-day':'');
...
function renderDashboard() {
    ...
    taskData.forEach((t, i) => {
        const dueDate = calculateDueDate(t.lastCompleted, t.frequencyDays, t.isOneTime);
        if (t.isOneTime && t.frequencyDays === 0) return;
        const status = getStatus(dueDate);
        if (status.sortValue <= 30) {
            const row = document.createElement('tr'); row.className = status.class;
            row.innerHTML = `<td>${formatDate(dueDate)}</td><td>${t.taskName}</td><td>${t.category}</td><td>${status.text}</td>
